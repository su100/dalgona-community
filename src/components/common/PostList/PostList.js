          ...
          {postList.length === 0 ? (
            ...
            postList.map((post) => {
              ...
              return currentPostId === post.id ? (
                <div className={noBorder ? 'post-list__item no-border' : 'post-list__item'} key={`${post.id}-pc`}>
        ...
        <div className="post-list__container">
          {postList.length === 0 ? (
            ...
            postList.map((post) => {
              ...
              if (type === 'list') {
                return currentPostId === post.id ? (
                  <div className={noBorder ? 'post-list__item no-border' : 'post-list__item'} key={post.id}>
                    <div className="post-list__item--left">
                      {imageURL !== '' && (
                        <div className="post-list__img">
                          <img src={imageURL} alt="post" />
                    ...
                    {hasReply && <div className="post-list__item--comment not-pc">{post.reply_count}</div>}
                  </div>
                ) : (
                  <Link
                    to={!request ? `${link}/${post.id}` : `${link}/${post.id}${request}`}
